...
class ArchitectSubAgent(BaseSubAgent):
    ...
    def process(self, input_data: Dict[str, Any]) -> SubAgentResult:
        ...
        if existing_knowledge:
            knowledge_coherence = self._check_knowledge_coherence(content, existing_knowledge)
        ...
        message = "設計整合性チェック完了" if all_passed else f"{len(warnings)}件の懸念事項があります"

        return SubAgentResult(
            status=status,
            data={
                'coherence_score': sum(1 for c in coherence_checks if c['passed']) / len(coherence_checks),
    ...
    def _check_title_content_coherence(self, title: str, content: str) -> Dict[str, Any]:
        """タイトルと内容の整合性をチェック"""
        ...
        # 一般的なストップワードを除外
        stop_words = {'の', 'に', 'を', 'は', 'が', 'で', 'と', 'から', 'まで', 'について', 'に関する'}
        ...
        # キーワードの含有率をチェック
        matched_keywords = sum(1 for keyword in title_keywords if keyword in content_lower)
        ...
        return {
            'check_name': 'タイトルと内容の整合性',
            'passed': passed,
            'message': f'タイトルキーワードの{int(match_rate*100)}%が内容に含まれています' if passed else
                      f'タイトルと内容の関連性が低い可能性があります（{int(match_rate*100)}%）',
            'details': {
    ...
    def _check_itsm_validity(self, itsm_type: str, content: str) -> Dict[str, Any]:
        ...
        return {
            'check_name': 'ITSMタイプの妥当性',
            'passed': matched,
            'message': f'ITSMタイプ "{itsm_type}" と内容が整合しています' if matched else
                      f'ITSMタイプ "{itsm_type}" と内容の整合性を確認してください',
            'details': {
                'itsm_type': itsm_type,
    ...
    def _check_knowledge_coherence(self, content: str, existing_knowledge: list) -> Dict[str, Any]:
        """既存ナレッジとの整合性をチェック"""
        # 既存ナレッジとの矛盾や重複をチェック
        # 簡易実装: 類似度が高すぎる場合は警告
        similar_count = len([k for k in existing_knowledge if self._calculate_similarity(content, k.get('content', '')) > 0.8])
        ...
        return {
            ...
            'passed': passed,
            'message': '既存ナレッジとの整合性OK' if passed else
                      f'{similar_count}件の類似ナレッジが存在します',
            'details': {
    ...
    def _check_design_principles(self, content: str) -> Dict[str, Any]:
        ...
        passed = passed_count >= len(principles) * 0.6  # 60%以上の原則に準拠していればOK
    ...
    def _generate_recommendations(self, warnings: list) -> list:
        ...
        for warning in warnings:
            ...
            if check_name == 'タイトルと内容の整合性':
                recommendations.append('タイトルを内容に即したものに修正することを推奨します')
            elif check_name == 'ITSMタイプの妥当性':
                recommendations.append('ITSMタイプの見直し、または内容の明確化を推奨します')
            elif check_name == '既存ナレッジとの整合性':
                recommendations.append('既存の類似ナレッジとの統合または差別化を検討してください')
            elif check_name == '設計原則への準拠':
                recommendations.append('原因・対策・影響範囲などの情報を追加することを推奨します')
