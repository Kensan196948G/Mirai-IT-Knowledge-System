...
class KnowledgeCuratorSubAgent(BaseSubAgent):
    ...
    def _extract_tags(self, title: str, content: str, itsm_type: str) -> List[str]:
        ...
        # 技術タグ
        tech_tags = {
            'ネットワーク': ['network', 'ネットワーク', 'lan', 'wan', 'vpn', 'dns', 'dhcp'],
            'データベース': ['database', 'db', 'データベース', 'sql', 'mysql', 'postgresql', 'oracle'],
            'サーバー': ['server', 'サーバー', 'サーバ', 'apache', 'nginx', 'iis'],
            'セキュリティ': ['security', 'セキュリティ', '脆弱性', 'firewall', 'ファイアウォール'],
            'バックアップ': ['backup', 'バックアップ', 'restore', 'リストア', '復元'],
            'パフォーマンス': ['performance', 'パフォーマンス', '性能', '遅延', 'レスポンス'],
            'アクセス権限': ['permission', '権限', 'access', 'アクセス', 'authentication', '認証'],
            ...
            '仮想化': ['virtual', '仮想', 'vm', 'vmware', 'hyper-v', 'docker', 'kubernetes'],
        ...
        if itsm_type == 'Incident':
            if any(word in text for word in ['緊急', '重大', 'クリティカル', 'critical']):
                tags.append('緊急対応')
            ...
            if any(word in text for word in ['デプロイ', 'deploy', 'リリース', 'release']):
                tags.append('リリース')
    ...
    def _extract_keywords(self, content: str) -> List[str]:
        ...
        # ストップワード（除外する一般的な単語）
        stop_words = {
            'の', 'に', 'を', 'は', 'が', 'で', 'と', 'から', 'まで', 'より', 'など',
            'この', 'その', 'あの', 'どの', 'ある', 'いる', 'する', 'なる', 'れる',
            'です', 'ます', 'した', 'ました', 'ている', 'される', 'として', 'について'
        }
    ...
    def _evaluate_importance(self, title: str, content: str, itsm_type: str) -> Dict[str, Any]:
        """重要度評価"""
        score = 0.5  # 基準スコア
        ...
        # 緊急度による加点
        urgent_keywords = ['緊急', '重大', 'クリティカル', 'critical', '障害', 'down', 'ダウン']
    ...
    def _generate_metadata(
        ...
        return {
            ...
            'has_list': '-' in content or '*' in content or re.search(r'\d+\.', content) is not None
        }
