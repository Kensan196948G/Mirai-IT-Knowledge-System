...
class DevOpsSubAgent(BaseSubAgent):
    ...
    def _extract_technical_elements(self, content: str) -> List[Dict[str, str]]:
        ...
        tech_categories = {
            ...
            'データベース': ['mysql', 'postgresql', 'oracle', 'mongodb', 'redis', 'db', 'database'],
            'Webサーバー': ['apache', 'nginx', 'iis', 'tomcat'],
            'プログラミング言語': ['python', 'java', 'javascript', 'php', 'ruby', 'go', 'c++'],
            ...
            'ネットワーク': ['vpn', 'dns', 'dhcp', 'firewall', 'load balancer', 'proxy'],
    ...
    def _evaluate_automation_potential(self, content: str, itsm_type: str) -> Dict[str, Any]:
        """自動化可能性を評価"""
        score = 0.0
        reasons = []
        content_lower = content.lower()
        ...
        # 繰り返し作業の検出
        repetitive_keywords = ['定期', '毎日', '毎週', '毎月', '繰り返し', 'daily', 'weekly']
        if any(keyword in content_lower for keyword in repetitive_keywords):
            score += 0.3
            reasons.append('定期的な作業である')

        # 手順が明確
        if any(pattern in content for pattern in ['1.', '2.', '3.', '手順', 'ステップ']):
            score += 0.2
        ...
        # スクリプト化可能な操作
        scriptable_keywords = ['コマンド', 'command', 'スクリプト', 'script', 'api', 'curl', 'ssh']
    ...
    def _suggest_improvements(self, content: str, automation_potential: Dict[str, Any]) -> List[str]:
        """改善提案を生成"""
        improvements = []
        content_lower = content.lower()
        ...
        if automation_potential['score'] >= 0.4:
            improvements.append(f"自動化の検討: {automation_potential['recommendation']}")
        ...
        if 'コマンド' in content_lower and '```' not in content:
            improvements.append('コマンドはコードブロック（```）で記載すると可読性が向上します')

        # 監視・アラート
        if any(word in content_lower for word in ['障害', 'エラー', 'ダウン']) and '監視' not in content_lower:
            improvements.append('同様の事象を早期検知するための監視・アラート設定を検討してください')

        # テスト
        if any(word in content_lower for word in ['変更', 'リリース', 'デプロイ']) and 'テスト' not in content_lower:
            improvements.append('変更前にテスト環境での検証を実施することを推奨します')

        # バックアップ
        if any(word in content_lower for word in ['削除', '変更', '更新']) and 'バックアップ' not in content_lower:
