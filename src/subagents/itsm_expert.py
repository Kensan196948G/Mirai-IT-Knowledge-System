...
class ITSMExpertSubAgent(BaseSubAgent):
    ...
    def __init__(self):
        super().__init__(name="itsm_expert", role="compliance", priority="high")
        # ITSM原則定義
    ...
    def _detect_deviations(self, content: str, itsm_type: str) -> List[Dict[str, Any]]:
        ...
        common_deviations = [
            ...
            },
            {
                ...
                "condition": lambda: itsm_type == "Incident"
                and "原因" not in content_lower,
            },
        ]
    ...
    def _evaluate_best_practices(self, content: str, itsm_type: str) -> Dict[str, Any]:
        ...
        if itsm_type == "Change":
            ...
            if any(
                word in content_lower for word in ["バックアップ", "backup", "退避"]
            ):
        ...
        return {"practices_followed": best_practices, "count": len(best_practices)}
    ...
    def _generate_recommendations(
        ...
        if best_practices["count"] < 2:
            recommendations.append(
                "時刻、担当者、確認プロセスなどの情報を追加することを推奨します"
            )

        return recommendations

    def _calculate_compliance_score(
        self, principle_checks: List[Dict[str, Any]], deviations: List[Dict[str, Any]]
    ) -> float:
        """ITSM準拠スコアを計算"""
        ...
        deviation_penalty = (len(error_deviations) * 0.2) + (
            len(warning_deviations) * 0.1
        )
