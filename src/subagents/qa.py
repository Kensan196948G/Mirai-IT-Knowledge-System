...
class QASubAgent(BaseSubAgent):
    ...
    def __init__(self):
        super().__init__(name="qa", role="quality_validation", priority="high")
    ...
    def _check_completeness(self, title: str, content: str) -> Dict[str, Any]:
        ...
        title_check = {
            ...
            "message": (
                "タイトルの長さが適切"
                if 10 <= len(title) <= 100
                else "タイトルが短すぎるか長すぎます（推奨: 10-100文字）"
            ),
        }
        ...
        specifics_check = {
            ...
            "message": (
                "具体的な情報が含まれています"
                if has_specifics
                else "時刻、対象システム、エラーメッセージなど具体的な情報を追加してください"
            ),
        }
    ...
    def _detect_duplicates(
        self, title: str, content: str, existing_knowledge: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """重複を検知"""
        similarities = []
        ...
        high_similarity_count = sum(
            1 for s in similarities if s["overall_similarity"] >= 0.8
        )
    ...
    def _calculate_quality_score(
        self, title: str, content: str, completeness: Dict[str, Any]
    ) -> Dict[str, Any]:
        """品質スコアを算出"""
        score = 0.0
        factors = []
    ...
    def _suggest_quality_improvements(
        self, completeness: Dict[str, Any], quality_score: Dict[str, Any]
    ) -> List[str]:
        """品質改善提案"""
        improvements = []
        ...
        if quality_score["score"] < 0.8:
            ...
            low_factors = [
                ...
                if f["contribution"] < f["weight"] * 0.5
            ]
            for factor in low_factors:
                if factor["factor"] == "可読性":
                    improvements.append(
                        "見出しや箇条書きを使って読みやすく構造化してください"
                    )
