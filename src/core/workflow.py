...
class WorkflowEngine:
    ...
    def process_knowledge(
        ...
        try:
            ...
            if pre_task_result.block_execution:
                raise ValueError(f"Pre-Task Hook でブロックされました: {pre_task_result.message}")

            # 2. 既存ナレッジを検索（重複チェック用）
            existing_knowledge = self.db_client.search_knowledge(
                query=title,
                limit=10
            )
            ...
            hook_results = self._execute_quality_hooks(
                {
                    ...
                    'itsm_expert_result': subagent_results.get('itsm_expert', {}).get('data', {}),
                    'documenter_result': subagent_results.get('documenter', {}).get('data', {})
                },
                execution_id
            )
            ...
            return {
                ...
                'subagent_results': subagent_results,
                'hook_results': hook_results,
                'post_task_assessment': post_task_result.details.get('overall_assessment', {}),
    ...
    def _aggregate_knowledge(
        ...
        try:
            # 検出された技術を抽出
            detected_techs = [tag for tag in tags if tag.lower() in ['flask', 'sqlite', 'python', 'apache', 'nginx']]
            ...
            if mcp_enrichments.get('related_memories'):
                insights.append(f"📚 過去の関連記憶: {len(mcp_enrichments['related_memories'])}件見つかりました")
            if mcp_enrichments.get('technical_documentation'):
                insights.append(f"📖 技術ドキュメント: {len(mcp_enrichments['technical_documentation'])}個の技術について参照可能")
    ...
    def _save_markdown(self, knowledge_id: int, knowledge: Dict[str, Any]) -> str:
        ...
        # データベースにパスを保存
        self.db_client.update_knowledge(
            knowledge_id,
