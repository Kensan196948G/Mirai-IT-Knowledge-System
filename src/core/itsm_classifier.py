...
class ITSMClassifier:
        ...
        return {
            ...
            'Release': {
                'primary_keywords': [
                    ...
                    '本番', 'production', 'リリースノート'
                ],
                'secondary_keywords': [
                    '機能', 'バージョン', 'version', 'ビルド', 'build',
                    'ロールアウト'
                ],
                'weight_primary': 0.7,
                'weight_secondary': 0.3
    ...
    def _calculate_score(self, text: str, rules: Dict[str, Any]) -> float:
        ...
        # 重み付けスコア
        total_score = (primary_score * weight_primary) + (secondary_score * weight_secondary)
