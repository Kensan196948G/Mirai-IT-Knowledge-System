...
class TestGenerateRecommendations:
    ...
    def test_recommendations_title_content_coherence(self, architect):
        """タイトルと内容の整合性に関する推奨事項"""
        # Given
        warnings = [{"check_name": "タイトルと内容の整合性"}]
    ...
    def test_recommendations_itsm_validity(self, architect):
        """ITSMタイプの妥当性に関する推奨事項"""
        # Given
        warnings = [{"check_name": "ITSMタイプの妥当性"}]
    ...
    def test_recommendations_knowledge_coherence(self, architect):
        """既存ナレッジとの整合性に関する推奨事項"""
        # Given
        warnings = [{"check_name": "既存ナレッジとの整合性"}]
...
class TestEdgeCases:
    ...
    def test_process_empty_itsm_type(self, architect):
        """空のITSMタイプ"""
        # Given
        input_data = {"title": "テスト", "content": "テスト内容", "itsm_type": ""}
    ...
    def test_process_with_many_existing_knowledge(self, architect):
        """大量の既存ナレッジ"""
        # Given
        existing = [{"id": f"KB{i:03d}", "content": f"内容{i}"} for i in range(100)]
        input_data = {
            "title": "テスト",
            "content": "テスト内容",
            "itsm_type": "Incident",
