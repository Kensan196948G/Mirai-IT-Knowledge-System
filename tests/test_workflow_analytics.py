...
class TestWorkflowEngineCoverage:
    ...
    def test_workflow_aggregate_knowledge(self, mock_db):
        ...
        # モックサブエージェントを作成
        mock_subagents = {}
        for name in ["architect", "knowledge_curator", "itsm_expert", "devops", "qa", "coordinator", "documenter"]:
            mock_subagents[name] = MagicMock()

        with patch.multiple(
            'src.core.workflow',
            ArchitectSubAgent=MagicMock(return_value=mock_subagents["architect"]),
            KnowledgeCuratorSubAgent=MagicMock(return_value=mock_subagents["knowledge_curator"]),
...
class TestAnalyticsEngineCoverage:
    ...
    def test_analytics_generate_recommendations(self, mock_feedback, mock_db):
        ...
        # インシデント多数シナリオをモック
        with patch.object(engine, 'analyze_incident_trends', return_value={'total_incidents': 60}):
            with patch.object(engine, 'analyze_problem_resolution_rate', return_value={'resolution_rate': 65}):
                with patch.object(engine, 'analyze_knowledge_quality', return_value={'summary_coverage': 75}):
